<script lang="ts">  
  ...
  async function renderD3(container, xattr, yattr) {
    ...
    let response_x = await fetch('http://localhost:5000/graphkeys?attr=' + xattr + '&tag=city_name');
    let graphkeys_x = await response_x.json();
    
    let response_y = await fetch('http://localhost:5000/graphkeys?attr=' + yattr + '&tag=city_name');
  ...
  async function renderMapbox(container, attr) {
    let response = await fetch('http://localhost:5000/mapdata?xattr=' + attr + '&tag=city_name');
    let response_json = await response.json();
