...
class OpenStackClient:
  ...
  @traced()
  async def init_unimelb_proxy(self):
    proc = subprocess.Popen([
      ...
      path.join(PATHS['ansible'], 'unimelb-proxy.yml')
    ])
    proc.wait()
  ...
  async def setup_docker_context(self):
    ...
    exists = self.docker_context_name in subprocess.check_output(['docker', 'context', 'ls', '-q']).decode().splitlines()
...
async def main():
  ...
  await client.create_stack(path.join(PATHS['heat'], f'{args.stack}.yaml'), {'key_name': 'alaroldai_haliax'})
  await client.write_hosts_file()
  await client.init_unimelb_proxy()
